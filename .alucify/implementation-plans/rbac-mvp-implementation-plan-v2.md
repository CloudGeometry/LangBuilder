...
- `/api/v1/rbac/check-permissions-batch` - Batch permission check endpoint for list views
...
- Batch permission check endpoint to reduce N+1 queries in list views
- TanStack Query caching (staleTime: 5 minutes) for permission checks
...
- Cache expires after 1 hour or on service reinit
...
- New Nodes:
  ...
  - New: POST /api/v1/rbac/check-permissions-batch (logic)
- Modified Nodes: None
...
- PATCH rejects immutable assignments with 400 error
- DELETE rejects immutable assignments with 400 error
- All responses follow OpenAPI spec
- Request validation with Pydantic
- Error responses include descriptive messages
- Batch permission endpoint reduces list view queries by N:1
...
**Scope and Goals:**
Update flow and project list endpoints to filter results by Read permission. Only return flows/projects the user has Read permission for. Implement batch permission checking to optimize performance for large lists.

**Impact Subgraph:**
- Modified Nodes:
  - `nl0005`: List Flows Endpoint Handler (logic)
  - `nl0008`: List Projects Endpoint Handler (logic)
- Edges: List endpoints now check Read permission for each resource

**Architecture & Tech Stack:**
- Framework: FastAPI with RBACService dependency
- Libraries: TanStack Query (frontend) with caching
- Patterns: Async batch permission checks, in-memory filtering
- File Locations:
  - `/home/nick/LangBuilder/src/backend/base/langbuilder/api/v1/flows.py`
  - `/home/nick/LangBuilder/src/backend/base/langbuilder/api/v1/projects.py`
...
- Batch permission check reduces N+1 query problem
...
- Batch permission checks reduce API calls
...
**Scope and Goals:**
Implement the main assignment list view showing all User:Role:Scope assignments. Support filtering by User, Role, and Scope. Show inherited roles from Project to Flow with clear messaging. Provide inline delete actions.
...
export function AssignmentListView({ onEdit, onDelete }: AssignmentListViewProps) {
    ...
    const { data: assignments } = useAssignments(filters)
    ...
    const columns = useMemo(() => [
        {
            ...
            header: "User",
            cell: (info) => info.row.original.user?.name || "Unknown"
        },
        {
        ...
        },
        {
            ...
            header: "Scope",
            cell: (info) => {
                if (info.row.original.scope_id) {
                    return `${info.row.original.scope_type} (${info.row.original.scope_id})`
                }
                return "Global"
            }
        },
            ...
            cell: (info) => (
                <div className="flex gap-2">
                    <Button
                        ...
                        disabled={info.row.original.is_immutable}
                    >
                        Edit
                    </Button>
                    <Button
    ...
    return (
        <div>
            <div className="flex gap-4 mb-4">
                <Select value={filters.user_id} onValueChange={(val) =>
                ...
                <Select value={filters.role_id} onValueChange={(val) =>
                    setFilters({...filters, role_id: val})
                }>
                    <SelectTrigger>
...
**Success Criteria:**
- All assignments displayed in table format
- Filtering by user, role, scope works
- Delete actions work for non-immutable assignments
- Edit actions trigger modal (Task 4.3)
- Immutable assignments show as disabled
- Inherited role message clearly displayed
- Real-time updates when assignments change
- Unit tests verify filtering logic
- Integration tests verify data fetching and updates

---

#### Task 4.3: Create Assignment Creation and Edit Wizard Modal

**Scope and Goals:**
Implement multi-step wizard modal for creating and editing role assignments. Steps: Select User → Select Scope → Select Role → Confirm. Validate that immutable assignments cannot be edited, and provide clear error messages for invalid operations.

**Impact Subgraph:**
- New Nodes:
  - `ni0085`: CreateAssignmentModal (interface)
- Modified Nodes: None
- Edges: RBACManagementPage uses CreateAssignmentModal

**Architecture & Tech Stack:**
- Framework: React with TypeScript
- Libraries: Radix UI (Dialog, Select), React Hook Form for wizard state
- Libraries: Zod for validation
- Patterns: Multi-step form with wizard, controlled inputs
- File Locations:
...
interface CreateAssignmentModalProps {
    isOpen: boolean
    assignment?: Assignment  // For edit mode
    onClose: () => void
...
export function CreateAssignmentModal({
    ...
    const [step, setStep] = useState<"user" | "scope" | "role" | "confirm">("user")
    ...
    const { mutate: createAssignment } = useMutation({
        mutationFn: async (data) => {
            const response = await fetch("/api/v1/rbac/assignments", {
    ...
    const handleNext = () => {
        const steps: ("user" | "scope" | "role" | "confirm")[] = ["user", "scope", "role", "confirm"]
    ...
    const handleSubmit = () => {
        if (assignment && assignment.is_immutable) {
            setError("Cannot modify immutable assignment")
            return
        }
            ...
            <DialogContent>
                ...
                {step === "scope" && (
                    <ScopeSelectionStep
                        ...
                        scopeId={formData.scope_id}
                        onScopeTypeChange={(type) =>
                            setFormData({...formData, scope_type: type, scope_id: null})
                        }
                        onScopeIdChange={(id) =>
                            setFormData({...formData, scope_id: id})
                        }
                    />
                )}
...
- All steps (user, scope, role, confirm) functional
- Validation on each step prevents invalid data
- Immutable assignments show error message
...
**Scope and Goals:**
Create custom React hook and component for permission checking in the frontend. `usePermission` hook provides methods to check permissions for current user. `RBACGuard` component conditionally renders content based on permissions.
...
- Modified Nodes: None
- Edges: Frontend components use these for permission checks

**Architecture & Tech Stack:**
- Framework: React with TypeScript
- Libraries: TanStack Query for caching, React Context for auth state
...
export function usePermission() {
    ...
    const checkPermission = useCallback(
        async (permission: string, scopeType: string, scopeId?: UUID) => {
            if (!user) return false

            try {
                const response = await fetch(
                    `/api/v1/rbac/check-permission?permission=${permission}&scope_type=${scopeType}&scope_id=${scopeId || ""}`,
                    { method: "GET" }
                )
    ...
    return {
        canRead: (scopeId?: UUID) => checkPermission("Read", "flow", scopeId),
        canCreate: (scopeId?: UUID) => checkPermission("Create", "project", scopeId),
        canUpdate: (scopeId?: UUID) => checkPermission("Update", "flow", scopeId),
        canDelete: (scopeId?: UUID) => checkPermission("Delete", "flow", scopeId),
        checkPermission,
    }
}
```
...
export function RBACGuard({
    ...
    }, [permission, scopeType, scopeId, checkPermission])
...
**Success Criteria:**
- usePermission hook correctly checks permissions
- RBACGuard conditionally renders content
- Permissions cached for 5 minutes
- TanStack Query handles cache invalidation
- Error handling for failed permission checks
- Unit tests verify hook logic
- Integration tests verify guard behavior

---

#### Task 4.5: Implement Read-Only Editor Mode for Users Without Update Permission

**Scope and Goals:**
Update Flow and Project editor pages to detect when user has Read but not Update permission, and display editor in read-only mode. Hide edit buttons and disable form inputs.

**Impact Subgraph:**
- Modified Nodes:
  - `ni0006`: CollectionPage (implement read-only checks)
  - `ni0009`: FlowPage (implement read-only editor mode)
- Edges: Editor pages use usePermission hook for permission checks

**Architecture & Tech Stack:**
- Framework: React with TypeScript
- Libraries: usePermission hook from Task 4.4
- Patterns: Conditional rendering based on permissions
- File Locations:
  - `/home/nick/LangBuilder/src/frontend/src/pages/FlowPage/index.tsx`
  - `/home/nick/LangBuilder/src/frontend/src/components/FlowEditor/index.tsx`

**Implementation Details:**

```typescript
export function FlowPage({ flowId }: FlowPageProps) {
    ...
    useEffect(() => {
        if (flow) {
            canRead(flow.id).then(can_read => {
                if (!can_read) {
                    // No read permission, redirect
                    navigate("/flows")
                }
            })

            canUpdate(flow.id).then(can_update => {
                setIsReadOnly(!can_update)
            })
        }
    }, [flow])
...
**Scope and Goals:**
Create performance tests to benchmark RBAC operations against PRD requirements. Optimize permission checks, API endpoints, and frontend queries. Ensure all latency targets are met: <50ms for CanAccess (p95), <200ms for assignment API (p95), <2.5s for editor load (p95).
...
async def test_batch_permission_check_optimization(benchmark, client, user):
    """Benchmark batch permission check endpoint."""
...
- Batch permission check 10x faster than N individual checks
...
4. **Error Scenarios**
   - Attempting to delete Starter Project assignment shows error
...
- **Performance Tests** (Task 5.2): Latency benchmarking
...
- **Status**: ADDRESSED
- **Resolution**: Added POST /api/v1/rbac/check-permissions-batch endpoint in Task 2.2
...
The refined implementation plan addresses all 4 audit recommendations while maintaining the comprehensive structure of the original plan. Key additions:

1. **Task 1.7** adds explicit data migration for existing users with dry-run and rollback support
2. **Task 5.3** adds monitoring, observability, and health checks for 99.9% availability
3. **Batch permission endpoint** in Task 2.2 and optimization details in Task 3.1
4. **Enhanced error messages** throughout frontend and API tasks

The plan now provides a complete, production-ready RBAC implementation with strong operational support and clear migration path from current state.
