"""CG-Jira Authentication Component

Handles authentication with Jira Cloud using API tokens.
Returns auth credentials consumed by all CG-Jira components.
...
class CGJiraAuthComponent(Component):
    """CG-Jira Authentication Component.

    Handles authentication with Jira Cloud or Jira Server/Data Center using various methods.
    Returns a Data object containing authentication credentials that can be passed to other CG-Jira components.
    """

    display_name = "CG-Jira Auth"
    description = "Authenticate with Jira Cloud or Server using API tokens. Provides credentials to all CG-Jira components."
