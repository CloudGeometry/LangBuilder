...
# ── Cost fallback ────────────────────────────────────────────────────────────
# Temporary: LangWatch computes cost server-side but doesn't expose it via the
# API (canSeeCosts permission blocks API key auth).  Until they fix this, we
# compute cost locally using the same per-token rates from their llmModels.json.
# Rates: (input_cost_per_token, output_cost_per_token)
_MODEL_COST_PER_TOKEN: dict[str, tuple[float, float]] = {
    # Anthropic
    "anthropic/claude-opus-4.6": (5e-06, 2.5e-05),
    "anthropic/claude-opus-4.5": (5e-06, 2.5e-05),
    "anthropic/claude-opus-4.1": (1.5e-05, 7.5e-05),
    "anthropic/claude-opus-4": (1.5e-05, 7.5e-05),
    "anthropic/claude-sonnet-4.6": (3e-06, 1.5e-05),
    "anthropic/claude-sonnet-4.5": (3e-06, 1.5e-05),
    "anthropic/claude-sonnet-4": (3e-06, 1.5e-05),
    "anthropic/claude-3.7-sonnet": (3e-06, 1.5e-05),
    "anthropic/claude-3.5-sonnet": (6e-06, 3e-05),
    "anthropic/claude-haiku-4.5": (1e-06, 5e-06),
    "anthropic/claude-3.5-haiku": (8e-07, 4e-06),
    "anthropic/claude-3-haiku": (2.5e-07, 1.25e-06),
    # OpenAI
    "openai/gpt-4o": (2.5e-06, 1e-05),
    "openai/gpt-4o-mini": (1.5e-07, 6e-07),
    "openai/gpt-4-turbo": (1e-05, 3e-05),
    "openai/gpt-4.1": (2e-06, 8e-06),
    "openai/gpt-4.1-mini": (4e-07, 1.6e-06),
    "openai/gpt-4.1-nano": (1e-07, 4e-07),
    "openai/gpt-5": (1.25e-06, 1e-05),
    "openai/gpt-5-mini": (2.5e-07, 2e-06),
    "openai/o3": (2e-06, 8e-06),
    "openai/o3-mini": (1.1e-06, 4.4e-06),
    "openai/o4-mini": (1.1e-06, 4.4e-06),
    # Google
    "gemini/gemini-2.5-pro": (1.25e-06, 1e-05),
    "gemini/gemini-2.5-flash": (3e-07, 2.5e-06),
}


def _estimate_cost(model: str | None, prompt_tokens: int | None, completion_tokens: int | None) -> float:
    """Estimate cost from model + tokens using LangWatch's published rates."""
    if not model or prompt_tokens is None:
        return 0.0
    rates = _MODEL_COST_PER_TOKEN.get(model)
    if not rates:
        return 0.0
    return (prompt_tokens * rates[0]) + ((completion_tokens or 0) * rates[1])
...
class LangWatchService:
    ...
    def _parse_trace(trace: dict) -> dict | None:
        ...
        try:
            ...
            # Cost: prefer LangWatch's total_cost, fall back to local estimation
            ...
            # If LangWatch didn't provide cost, estimate locally from tokens + model
            if cost_usd == 0.0 and model and prompt_tokens is not None:
                cost_usd = _estimate_cost(model, int(prompt_tokens), int(completion_tokens) if completion_tokens else 0)
    ...
    async def _filter_by_ownership(
        ...
        # Filter traces — prefer flow_id match (rename-safe), fall back to name
        filtered: list[dict] = []
        for trace in traces:
            metadata = trace.get("metadata") or {}

            # Primary: match by flow_id (survives renames)
            trace_flow_id = metadata.get("flow_id")
            if trace_flow_id:
                try:
                    if UUID(trace_flow_id) in allowed_flow_ids:
                        filtered.append(trace)
                        continue
                except (ValueError, AttributeError):
                    pass  # malformed UUID, fall through to name matching

            # Legacy: match by name (for traces created before flow_id was added)
            labels: list = metadata.get("labels") or []
            flow_name = next(
                (lbl[6:] for lbl in labels if isinstance(lbl, str) and lbl.startswith("Flow: ")),
                None,
